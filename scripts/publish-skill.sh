...
SKILL_NAME="hello-worker"
WORK_DIR="$(mktemp -d /tmp/hello-worker-skill-XXXXXX)"
trap 'rm -rf "$WORK_DIR"' EXIT
TS="$(date +%Y%m%d%H%M%S)"
RELEASE_PATH="hello-worker/release"
...
"$ROOT_DIR/scripts/upload-file.sh" --file "$ROOT_DIR/skills/hello-worker/scripts/run.sh" --name "${SKILL_NAME}.sh" >/dev/null
echo "SKILL_INSTALL_COMMAND=bash <(curl -fsSL 'https://skill.vyibc.com/install-${SKILL_NAME}.sh?ts=${TS}')"
echo "CLI_COMMAND=bash <(curl -fsSL https://skill.vyibc.com/${SKILL_NAME}.sh) --mode=hello"
